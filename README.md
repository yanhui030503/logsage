...
- **历史页 `/history`**
  - 查看历史分析记录
  - **支持搜索（按标题/日志片段）**
  - **支持分类标签筛选（NPE / Bean / Port / SQL / Config）**
  - 紧凑列表展示：时间 + 标题 + 分类 badge + 最高置信原因
- **报告详情**: 查看完整的分析报告详情
- **卡片式 UI**: 美观的分析结果展示，包括：
  - TL;DR 摘要卡片
...
### 输出质量与专业性
- **基于证据的分析展示**：Top Causes 附带可展开的 evidence 引用块
- **日志关键行高亮**（Exception / Caused by / NPE / BeanCreationException 等）
- **错误自动分类升级**：NPE / Bean / Port / SQL / Config 等，并给出典型修复路径

### 智能交互
- **当日志信息不足时，自动生成“需要用户补充的信息清单”**
- **Analyze 页面以问答形式展示，可一键复制**

### 成本与稳定性
- **缓存命中不扣每日分析次数，未命中才计数**
- **Analyze 页面提示本次是否命中缓存**
- **增加请求防抖与后端幂等，避免重复提交**
- **持续迭代中**
